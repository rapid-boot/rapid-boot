<#--noinspection ALL-->
<!--output: frontend/views/${changeClassName}/index.vue-->
  ...
  <#if betweens??>
    ...
    <#if columns??>
      <#list columns as column>
        <#if column.formShow>
          <el-form-item label="<#if column.remark != ''>${column.remark}<#else>${column.changeColumnName}</#if>"<#if column.notNull> prop="${column.changeColumnName}"</#if>>
...
export default {
  ...
  cruds() {
    return CRUD({ title: '${apiAlias}', url: 'api/${changeClassName}/list', idField: '${pkChangeColName}', sort: '${pkChangeColName},desc', crudMethod: { ...crud${className} }})
  },
  data() {
    return {
      permission: {
